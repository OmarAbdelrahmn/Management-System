...
public class PublicRelationsMediaUiService(IPublicRelationsMediaService service)
{
    ...
    public async Task<(bool Success, string Message)> UpdateContentStatusAsync(int id, WebsiteContentStatus status, CancellationToken cancellationToken = default) => ToUi(await service.UpdateContentStatusAsync(id, new UpdateWebsiteContentStatusRequest(status, DateTime.UtcNow.AddHours(3)), cancellationToken), "تم تحديث حالة المحتوى.");
