...
public class MovementMaintenanceUiService(IMovementMaintenanceService service)
{
    ...
    public async Task<(bool Success, string Message)> MarkOverdueAssignmentsAsync(string? note = null, CancellationToken cancellationToken = default) => ToUi(await service.MarkOverdueAssignmentsAsync(new MarkOverdueVehicleAssignmentsRequest(DateTime.UtcNow.AddHours(3), note), cancellationToken), "تم تحديث حركات السيارات المتأخرة.");
